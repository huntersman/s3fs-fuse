...
static int doRelease(const char* _path, struct fuse_file_info* fi);
...
// NFS高可用时，新节点首先执行open，在缓冲区创建占位文件，然后执行write，执行方法填充，flush，release
static int s3fs_open(const char* _path, struct fuse_file_info* fi)
{
    ...
    headers_t    meta;
    // no truncate cache
    if(0 != (result = get_object_attribute(path, NULL, &meta, true, NULL, true))){
        return result;
    }
...
static int clearCache(const char* _path){
    ...
    if(it != pathToCacheKey.end()) {
        ...
        if(cache_it != cache_map.end()) {
            if (cache_it->second){
                S3FS_PRN_DBG("Clear cache,path=%s",path);
...
static int s3fs_readdir(const char* _path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    int* fuse = (int*)(buf + 40);
    cache_key += *fuse;
    pathToCacheKey[tmpPath] = cache_key;
    std::map<std::string, struct moss_cache*>::iterator cache_it;
